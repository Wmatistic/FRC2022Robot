...
//   private final PigeonIMU m_pigeon = new PigeonIMU(DRIVETRAIN_PIGEON_ID);
  ...
  private final Mk3ModuleConfiguration swerveModuleConfig = new Mk3ModuleConfiguration();

  private ChassisSpeeds m_chassisSpeeds = new ChassisSpeeds(1.0, 1.0, 1.0);

  public DrivetrainSubsystem() {
    ShuffleboardTab tab = Shuffleboard.getTab("Drivetrain");

    // Swerve Module Current Limit
    // Nominal seems to be limited to 7 to keep the bot running during extended periods
    swerveModuleConfig.setSteerCurrentLimit(20.0);
    swerveModuleConfig.setDriveCurrentLimit(40.0);
    swerveModuleConfig.setNominalVoltage(7);
...
// //     // FIXME Uncomment if you are using a NavX
  ...
  public void periodic() {
    ...
    m_frontLeftModule.set((states[0].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND * MAX_VOLTAGE)/.5, states[0].angle.getRadians());
    m_frontRightModule.set((states[1].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND * MAX_VOLTAGE)/.5, states[1].angle.getRadians());
    m_backLeftModule.set((states[2].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND * MAX_VOLTAGE)/.5, states[2].angle.getRadians());
    m_backRightModule.set((states[3].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND * MAX_VOLTAGE)/.5, states[3].angle.getRadians());
  }
}
